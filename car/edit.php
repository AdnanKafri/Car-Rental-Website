...
if(isset($_POST['ok']))
{
    ...
    $desc=$_POST['desc'];
    $q = "UPDATE `cars` 
                SET brand=?, model=?, color=?, year=?, price=?, description=?
                WHERE car_id = '$id'";
    $ps = $pdo->prepare($q);
    if($ps){
        $ps->bindParam(1, $brand);
        $ps->bindParam(2, $model);
        $ps->bindParam(3, $color);
        $ps->bindParam(4, $date);
        $ps->bindParam(5, $price);
        $ps->bindParam(6, $desc);
        $ps -> execute();
...
<form action="edit.php" method="post">
    ...
    <input type="submit" name="ok" value="تعديل" required>
